...
private List<Boolean> getAsBooleans() {
    ...
    public static void writeToBuf(net.minecraft.network.FriendlyByteBuf buf, Sequence sequence) {
        buf.writeVarInt(sequence.length());
        for (int i = 0; i < sequence.length(); i++) {
            buf.writeBoolean(sequence.get(i));
        }
    }

    public static Sequence readFromBuf(net.minecraft.network.FriendlyByteBuf buf) {
        int length = buf.readVarInt();
        Sequence seq = new Sequence();
        for (int i = 0; i < length; i++) {
            seq.append(buf.readBoolean());
        }
        return seq;
    }
