...
public class OrbBlock extends PulseBlock {
    ...
    public static final RegistryObject<ProtocolCommand> SPAWN = ProtocolCommands.COMMANDS.register("orb/spawn", () ->
    ...
    private static void playCommandSound(ExecutionContext context, Orb orb) {
        context.block().getLevel().playSound(null, context.block().getBlockPos(), ModSounds.ORB_COMMAND.get(), SoundSource.BLOCKS);
        orb.triggerConfirmSound(10);
    }

    public static final RegistryObject<ProtocolCommand> MOVE_RELATIVE_X = ProtocolCommands.COMMANDS.register("orb/move_relative_x", () ->
            new ProtocolCommand(1) {
                ...
                public void run(ExecutionContext context) {
                    if(context.block() instanceof OrbBlockEntity orb_machine) {
                        ...
                        orb.addDestination(context.params().get(0),0,0, true);
                        playCommandSound(context, orb);
                    }
                }
            });

    public static final RegistryObject<ProtocolCommand> MOVE_RELATIVE_Y = ProtocolCommands.COMMANDS.register("orb/move_relative_y", () ->
            new ProtocolCommand(1) {
                @Override
                public void run(ExecutionContext context) {
                    if(context.block() instanceof OrbBlockEntity orb_machine) {
                        ...
                        orb.addDestination(0, context.params().get(0), 0, true);
                        playCommandSound(context, orb);
                    }
                }
            });

    public static final RegistryObject<ProtocolCommand> MOVE_RELATIVE_Z = ProtocolCommands.COMMANDS.register("orb/move_relative_z", () ->
            new ProtocolCommand(1) {
                @Override
                public void run(ExecutionContext context) {
                    if(context.block() instanceof OrbBlockEntity orb_machine) {
                        ...
                        orb.addDestination(0, 0, context.params().get(0), true);
                        playCommandSound(context, orb);
                    }
                }
            });

    public static final RegistryObject<ProtocolCommand> MOVE_TO = ProtocolCommands.COMMANDS.register("orb/move_to", () ->
            new ProtocolCommand(3) {
                ...
                public void run(ExecutionContext context) {
                    if(context.block() instanceof OrbBlockEntity orb_machine) {
                        ...
                        orb.addDestination(pos.getX() + context.params().get(0), pos.getY() + context.params().get(1), pos.getZ() + context.params().get(2), false);
                        playCommandSound(context, orb);
                    }
                }
            });

    public static final RegistryObject<ProtocolCommand> GRAB = ProtocolCommands.COMMANDS.register("orb/grab", () ->
            new ProtocolCommand(0) {
                @Override
                public void run(ExecutionContext context) {
                    if(context.block() instanceof OrbBlockEntity orb_machine) {
    ...
    public static final RegistryObject<ProtocolCommand> TOGGLE_PEN = ProtocolCommands.COMMANDS.register("orb/toggle_pen", () ->
            new ProtocolCommand(0) {
                @Override
                public void run(ExecutionContext context) {
                    if(context.block() instanceof OrbBlockEntity orb_machine) {
    ...
    public static final RegistryObject<ProtocolCommand> TOGGLE_PROJECTILE = ProtocolCommands.COMMANDS.register("orb/toggle_projectile", () ->
            new ProtocolCommand(0) {
                @Override
                public void run(ExecutionContext context) {
                    if(context.block() instanceof OrbBlockEntity orb_machine) {
