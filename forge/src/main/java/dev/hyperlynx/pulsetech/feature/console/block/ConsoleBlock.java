...
public class ConsoleBlock extends PulseBlock implements EntityBlock {
    ...
    protected @NotNull InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        if (player instanceof ServerPlayer splayer && level.getBlockEntity(pos) instanceof ConsoleBlockEntity console) {
            ModMessages.CHANNEL.send(PacketDistributor.PLAYER.with(() -> splayer), new OpenConsoleMessage(pos, console.getPriorLinesOrEmpty(), console.getCommandBoxText()));
        }
    ...
    protected InteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        if(stack.is(ModItems.DATASHEET.get())) {
            return InteractionResult.FAIL;
        }
        if(level.isClientSide()) {
            return InteractionResult.PASS;
        }
        if (level.getBlockEntity(pos) instanceof ConsoleBlockEntity console && stack.is(ModItems.DATA_CELL.get())) {
            Macros console_macros = new Macros(console.getMacros(), console.getHiddenMacros());
            if (stack.hasTag() && stack.getOrCreateTag().contains("Macros")) {
                // There are stored macros that need to be synced to the console, and merged into the data on the item
                Macros stored_macros = dev.hyperlynx.pulsetech.core.program.Macros.fromNbt(stack.getOrCreateTag());
                if (console_macros.equals(stored_macros)) {
                    level.playSound(null, pos, ModSounds.BEEP.get(), SoundSource.PLAYERS, 1.0F, 0.9F + level.random.nextFloat() * 0.05F);
                ...
                merged.toNbt(stack.getOrCreateTag());
                ...
                console_macros.toNbt(stack.getOrCreateTag());
            }
