...
public class Orb extends Entity {
    ...
    protected void defineSynchedData() {
        entityData.define(PEN_DOWN, false);
        entityData.define(PROJECTILE, false);
        entityData.define(NEXT_DESTINATION, BlockPos.ZERO);
        entityData.define(RANGE_CENTER, BlockPos.ZERO);
        entityData.define(HAS_DESTINATION, false);
    }
    ...
    public void tick() {
        ...
        if(confirm_sound_timer == 0) {
            level().playSound(null, blockPosition(), ModSounds.ORB_CONFIRM.get(), SoundSource.NEUTRAL);
        ...
        if(isProjectile()) {
            ...
            if(!collided_entities.isEmpty()) {
                Entity victim = collided_entities.get(0);
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        if(tag.contains("Destinations")) {
            destinations = new ArrayDeque<>(Destination.CODEC.listOf().decode(NbtOps.INSTANCE, tag.get("Destinations")).getOrThrow(false, e -> {}).getFirst());
        }
        if(tag.contains("NextDestination")) {
            setNextDestination(NbtUtils.readBlockPos(tag.getCompound("NextDestination")));
        }
        ...
        setRangeCenter(NbtUtils.readBlockPos(tag.getCompound("RangeCenter")));
    }
