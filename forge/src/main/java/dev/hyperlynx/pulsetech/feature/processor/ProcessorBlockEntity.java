...
public class ProcessorBlockEntity extends ProtocolBlockEntity implements ProgramExecutor, DebuggerInfoSource {
    ...
    protected void saveAdditional(CompoundTag tag) {
        ...
        if(encode_result.resultOrPartial(e -> {}).isPresent()) {
            tag.put("ProgramEmitter", encode_result.getOrThrow(false, e -> {}));
        }
        if(!macros.isEmpty()) {
            MACRO_CODEC.encodeStart(NbtOps.INSTANCE, macros).result().ifPresent(encoded -> tag.put("macros", encoded));
        }
        tag.putString("OperationMode", operation_mode.name());
        tag.putInt("UnwrapCount", unwrap_count);
        if(!hidden_macros.isEmpty()) {
            HIDDEN_MACRO_CODEC.encodeStart(NbtOps.INSTANCE, hidden_macros).result().ifPresent(encoded -> tag.put("hidden_macros", encoded));
        }
    }
    ...
    protected void loadAdditional(CompoundTag tag) {
        ...
        if(decode_result.resultOrPartial(e -> {}).isPresent()) {
            emitter = decode_result.getOrThrow(false, e -> {}).getFirst();
        }
        if(tag.contains("macros")) {
            MACRO_CODEC.decode(NbtOps.INSTANCE, tag.get("macros")).result().ifPresent(pair -> macros = new HashMap<>(pair.getFirst()));
        }
        ...
        if(tag.contains("hidden_macros")) {
            HIDDEN_MACRO_CODEC.decode(NbtOps.INSTANCE, tag.get("hidden_macros")).result().ifPresent(pair -> hidden_macros = new HashSet<>(pair.getFirst()));
        }
    }
    ...
    public DebuggerInfoManifest getDebuggerInfoManifest() {
        ...
        protocol_manifest_list.remove(protocol_manifest_list.size() - 1);
